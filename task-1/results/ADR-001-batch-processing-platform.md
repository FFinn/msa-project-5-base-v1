...
|---|---|
| **F — Functionality** | Оркестрация пакетного пайплайна из CSV, PostgreSQL, Kafka и API; branching; fallback; уведомления |
| **U — Usability** | Наглядное состояние DAG, задач, retries и ошибок через UI |
| **R — Reliability** | Retry/backoff, локализация ошибок отдельных шагов, управляемые fallback-сценарии |
| **P — Performance** | Обработка порядка 1 млн записей за запуск без передачи всего объёма через Metadata DB/XCom |
| **S — Supportability** | Workflow as Code, Git-версионирование, task logs, история запусков, расширяемость provider-пакетами |
| **+ — Constraints / Compatibility** | Интеграции BigQuery, Redshift, Kafka, Spark, HTTP API; локальный POC и последующий cloud deployment |

### 2.3. Основной Use Case

| UC-ID | Название | Основной поток | Альтернативный поток | Особые требования |
|---|---|---|---|---|
| UC-01 | Запуск маркетингового batch-пайплайна | Trigger → чтение источников → анализ → ветвление → обработка выбранной ветки → сохранение результата → уведомление | Ошибка шага → retries → fallback/error branch → уведомление об ошибке | Наблюдаемость каждого шага, интеграции с внешними системами, отсутствие передачи больших payload через XCom |
...
Решение покрывает FURPS+ следующим образом:

- **F:** DAG, branching, Trigger Rules, callbacks и operators покрывают требуемую логику workflow;
- **R:** task-level retry/backoff и fallback-ветки позволяют восстанавливаться после локальных сбоев;
- **P:** тяжёлые вычисления делегируются SQL/BigQuery/Redshift/Spark, а Airflow передаёт metadata вместо больших payload;
- **S:** Workflow as Code, Web UI, история запусков и task logs упрощают сопровождение;
- **+:** provider-пакеты и Kubernetes/Cloud Composer дают нужную совместимость с целевой инфраструктурой.

## 5. Обоснование интеграций
...
- Airflow Metadata DB;
- Scheduler, Webserver и выбранный Executor;
- provider-пакеты для GCP/AWS/Kafka/Spark;
- SMTP или provider для уведомлений;
- внешний storage для промежуточных и итоговых данных;
- централизованный monitoring/logging stack в production;
- Secret Manager/Kubernetes Secrets для credentials.

## 12. Риски и ограничения

- Airflow не является low-latency streaming engine;
- self-hosted вариант требует сопровождения Scheduler, Metadata DB, Executor и Workers;
- Metadata DB становится критической инфраструктурной зависимостью;
- передача больших payload через XCom способна деградировать Metadata DB и Scheduler;
- Sensors в `poke` mode могут удерживать worker slots, поэтому для долгого ожидания предпочтительны reschedule/deferrable механизмы либо внешний trigger;
- производительность тяжёлых transform зависит не от самого Airflow, а от подключённых compute/storage систем.

## 13. Последствия решения
...
## 14. Критерии пересмотра ADR

Решение следует пересмотреть, если:

- pipeline станет преимущественно low-latency streaming, а не batch;
- останется только одна простая независимая задача по расписанию и Airflow станет избыточным;
- все процессы будут сосредоточены внутри Java/Spring и появится преимущество централизованной Spring Batch/SCDF платформы;
- нагрузочные тесты покажут необходимость отдельного distributed compute engine для значительной части transform.

## 15. POC, подтверждающий решение
