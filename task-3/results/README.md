...
- `k8s/init.sql` — тестовые данные;
- `k8s/output-pvc.yaml` — PVC для CSV в POC;
- `k8s/cronjob.yaml` — целевая конфигурация запуска ежедневно в 20:00;
- `k8s/cronjob-demo.yaml` — демонстрационный запуск каждую минуту;
- `DEMO.md` — сценарий проверки и перечень скриншотов.

## Ключевые решения

- `schedule: "0 20 * * *"` и явный `timeZone`;
- `concurrencyPolicy: Forbid` — новый ежедневный запуск не пересекается с предыдущим;
...
- имя файла содержит дату и обеспечивает понятный повторный запуск;
