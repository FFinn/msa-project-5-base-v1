...
Prometheus периодически опрашивает сервисы и забирает опубликованные ими метрики. Spring Boot публикует их через Micrometer/Actuator на `/actuator/prometheus`, а Kubernetes, Cloud SQL и очередь — через соответствующие экспортёры и готовые интеграции.
...
- `http_server_requests_seconds_bucket{method,uri,status,le}` — распределение длительности и расчёт 95-го и 99-го процентилей;
...
Для расчёта 95-го и 99-го процентилей по `*_bucket` в Spring Boot нужно включить публикацию гистограммы для `http.server.requests`:
...
Они показывают пропускную способность, устойчивость обработки и стоимость повторных попыток. Кроме того, по ним можно проверить требование бизнеса: среднее время обработки отчёта на 2 000 строк должно быть не более 30 секунд.

Для этого используется метка `size_class` с ограниченным набором значений: `le_2000`, `2001_10000`, `gt_10000`. Правило на 30 секунд применяется только к `size_class="le_2000"`, чтобы крупные файлы не искажали оценку типового отчёта.
